...
class FormInput
{
    ...
    private function basicInput() : string
    {
        ...
        ob_start();
        ?>
        <div class="col-md-<?= $colWidth ?> mb-3 pe-2">
            <label for="<?= $this->name ?>" class="form-label"><?= $this->label ?></label>
            <?php $this->maybePrintFormText(); ?>
            <input type="<?= $this->type ?>" id="<?= $this->name ?>" name="<?= $this->name ?>" class="form-control"<?= !empty($this->value) ? "value=\"{$this->value}\"" : "" ?><?= !empty($additionalAttrs) ? " $additionalAttrs" : "" ?><?= $this->formText ? " aria-describedby=\"form-text-{$this->name}\"" : "" ?>>
        </div>
        <?php
        return ob_get_clean();
    }
    ...
    private function selectInput() : string
    {
        ...
        <div class="col-md-<?= $colWidth ?> mb-3 pe-2">
            <label for="<?= $this->name ?>" class="form-label"><?= $this->label ?></label>
            <?php $this->maybePrintFormText(); ?>
            <select id="<?= $this->name ?>" name="<?= $this->name ?>" class="form-select"<?= !empty($additionalAttrs) ? " $additionalAttrs" : "" ?><?= $this->formText ? " aria-describedby=\"form-text-{$this->name}\"" : "" ?>>
    ...
    private function textareaInput() : string
    {
        ...
        ob_start();
        ?>
        <div class="col-md-<?= $this->baseWidth ?> mb-3">
            <label for="<?= $this->name ?>" class="form-label"><?= $this->label ?></label>
            <?php $this->maybePrintFormText(); ?>
            <textarea id="<?= $this->name ?>" name="<?= $this->name ?>" class="form-control"<?= !empty($additionalAttrs) ? " $additionalAttrs" : "" ?><?= $this->formText ? " aria-describedby=\"form-text-{$this->name}\"" : "" ?>><?= !empty($this->value) ? $this->value : "" ?></textarea>
        </div>
        <?php
        return ob_get_clean();
    }
